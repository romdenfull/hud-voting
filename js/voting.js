...
async function setupVoting() {
  ...
  const polls = await window.supabaseClient.from('polls').select('*').eq('is_active', true);
...
async function setupPollInteraction(pollId, options, session) {
  ...
  // Проверка: уже голосовал в этом опросе?
  ...
  options.forEach((opt, idx) => {
    ...
    btn.onclick = async () => {
      ...
      try {
        const { error } = await window.supabaseClient.from('votes').insert({
          poll_id: pollId,
...
async function renderResults(pollId, options, container) {
  ...
  const { data: votes } = await window.supabaseClient.from('votes').select('option_index, weight').eq('poll_id', pollId);
